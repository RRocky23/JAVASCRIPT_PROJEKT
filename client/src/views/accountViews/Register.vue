    ...
    <form method="post" @submit.prevent="handleSubmit">
      ...
      <div class="row mb-3">
        <div class="col-md-6">
          ...
          <input v-model="userModel.name" type="text" id="name" class="form-control" placeholder="Enter your name" required :disabled="loading"/>
          <small v-if="fieldErrors.name" class="text-danger">{{ fieldErrors.name }}</small>
        </div>
        <div class="col-md-6">
          <label for="surname" class="form-label">Surname</label>
          <input v-model="userModel.surname" type="text" id="surname" class="form-control" placeholder="Enter your surname" required :disabled="loading"/>
      ...
      <div class="mb-3 text-start">
        ...
        <input v-model="userModel.username" type="text" id="username" class="form-control" placeholder="Enter your username" required :disabled="loading"/>
        <small v-if="fieldErrors.username" class="text-danger">{{ fieldErrors.username }}</small>
      </div>

      <div class="mb-3 text-start">
        <label for="birthdate" class="form-label">Birth Date</label>
        <input v-model="userModel.birthDate" type="date" id="birthdate" class="form-control" required :disabled="loading"/>
        <small v-if="fieldErrors.birthDate" class="text-danger">{{ fieldErrors.birthDate }}</small>
      </div>

      <div class="mb-3 text-start">
        <label for="email" class="form-label">Email</label>
        <input v-model="userModel.email" type="email" id="email" class="form-control" placeholder="Enter your email" required :disabled="loading"/>
      ...
      <div class="d-flex justify-content-between mt-3">
        ...
        <button type="submit" class="btn btn-primary flex-grow-1 ms-2" :disabled="loading">Sign Up</button>
      </div>
    </form>
  </div>
</template>
...
const handleSubmit = async () => {
    ...
    try {
        await register(userModel); // używamy useAuth register
